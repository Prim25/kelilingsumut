...
import L, { GeoJSON as LGeoJSON, Layer, LeafletMouseEvent } from "leaflet";
...
function colorFromCategory(kategori?: string) {
  ...
  for (let i = 0; i < kategori.length; i++)
    hash = (hash * 31 + kategori.charCodeAt(i)) >>> 0;
  return colors[hash % colors.length];
}

const pointToLayer = (
  feature: Feature<Point, GeoProperties>,
  ...
  L.circleMarker(latlng, {
    ...
    fillOpacity: 0.9,
    color: "#111827",
    fillColor: colorFromCategory(feature.properties?.kategori),
  });

const style = (feature: Feature<Geometry, GeoProperties>) => ({
  color: colorFromCategory(feature.properties?.kategori),
  weight: 2,
  opacity: 0.9,
  fillOpacity: 0.3,
});
...
export default function MapClient({
    ...
    const features = raw.features.filter((f) => {
      ...
      const catOk =
        selectedCategory === "semua"
          ? true
          : String(p.kategori || "").toLowerCase() === wantedCat;
      const searchOk = String(p.nama || "")
        .toLowerCase()
        .includes(text);
  ...
  const sumutPoints = FEATURED_SUMUT.map(
  ...
  const onEachFeature = (
    ...
    const img = p.gambarUtama
      ? `<img src="${p.gambarUtama}" alt="${nama}" style="width:100%;max-height:140px;object-fit:cover;border-radius:8px;margin-bottom:8px;" />`
      : "";
    const desc = p.deskripsiUtama
      ? `<p style="margin:6px 0 0;line-height:1.3">${p.deskripsiUtama}</p>`
      : "";
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoadingPoi(true);
        const r = await fetch(
          `/api/nearby?lat=${active.lat}&lng=${active.lng}&radius=${
            (active.radiusKm ?? 6) * 1000
          }`,
          { signal: controller.signal }
        );
        const data: { hotels?: Poi[]; restaurants?: Poi[] } = await r.json();
      ...
      <aside className="border rounded-2xl p-5 bg-white h-[520px] overflow-y-auto shadow-sm font-display">
        {active ? (
          ...
          <div className="h-full grid place-items-center text-center text-sm text-gray-600">
            Klik salah satu pin{" "}
            <span className="font-medium">“Rekomendasi”</span> pada peta untuk
            zoom & lihat hotel/resto terdekat (ikon 🏨 & 🍽️).
          </div>
        )}
      </aside>
    </div>
...
//             <GeoJSON
//               ref={geoJsonRef}
//               data={filtered as FeatureCollection<Geometry, GeoProperties>}
//               onEachFeature={onEachFeature}
//               pointToLayer={pointToLayer}
//               style={style}
//             />
